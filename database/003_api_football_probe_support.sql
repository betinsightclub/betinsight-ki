-- BetInsight KI – API-Football probe support
-- Stand: 2026-09-06
-- Server-side only. No API key is stored in GitHub.

create extension if not exists http with schema extensions;

create table if not exists analysis.api_probe_results (
...
alter table analysis.api_probe_results enable row level security;
revoke all on analysis.api_probe_results from anon, authenticated;

create or replace function analysis.api_football_get(
  p_endpoint text,
...
begin
  ...
  from extensions.http((
    'GET',
    v_url,
    extensions.http_headers('x-apisports-key', v_key, 'accept', 'application/json'),
    null,
    null
