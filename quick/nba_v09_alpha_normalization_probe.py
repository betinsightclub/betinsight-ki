...
Z=Path('/tmp/anorm.sqlite.zip');R=Path('/tmp/anormdb');DB=R/'basketball-final.sqlite'
if not DB.exists():
...
def fit(tr,te,nominal,c,p):
    ae=nominal*c*((len(tr)/NREF)**p)
    sc=StandardScaler();A=sc.fit_transform(X.loc[tr.index]);B=sc.transform(X.loc[te.index]);m=Ridge(alpha=ae,fit_intercept=True).fit(A,tr.y.to_numpy());return m.predict(B),ae
...
def stage(cfac,powr):
    dev=[];ds=[];eff=[]
    for s in DEV:
        te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];pr,ae=fit(tr,te,300,cfac,powr);y=te.y.to_numpy();dev.append((pr,y));eff.append((s,len(tr),ae));ds.append(float(np.sqrt(np.mean(y*y))-np.sqrt(np.mean((y-pr)**2))))
    cc=np.concatenate([z[0] for z in dev]);yy=np.concatenate([z[1] for z in dev]);gates={g:bs(cc,yy,g) for g in TG};st=(min(ds),float(np.mean(ds)),float(np.median(ds)));d300=sum(abs(a-b) for a,b in zip(st,TA[300]));gerr=sum(sum(abs(a-b) for a,b in zip(gates[g],TG[g])) for g in TG)
    out=[];rmerr=berr=0
    for s,t in TO.items():
        te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];pr,ae=fit(tr,te,300,cfac,powr);y=te.y.to_numpy();rm=float(np.sqrt(np.mean((y-pr)**2)));b=bs(pr,y,.75);rmerr+=abs(rm-t[0]);berr+=sum(abs(a-bb) for a,bb in zip(b,t[1:]));out.append((s,len(tr),ae,rm,b))
    score=d300*5000+gerr*.12+rmerr*2400+berr*.12
    return score,st,gates,gerr,out,rmerr,berr,eff

res=[]
for cfac in np.arange(.60,1.401,.025):
    for powr in np.arange(-1.0,1.001,.125):
        z=stage(float(cfac),float(powr));res.append((z[0],float(cfac),float(powr),z))
res.sort(key=lambda x:x[0])
print('CANDIDATES',len(res),'BASE_RANK',next(i+1 for i,r in enumerate(res) if abs(r[1]-1)<1e-9 and abs(r[2])<1e-9))
print('\nTOP40')
for i,(sc,c,p,z) in enumerate(res[:40],1):print(i,'SCORE',sc,'C',c,'P',p,'DEV',z[1],'G75',z[2][.75],'GERR',z[3],'RMERR',z[5],'BETERR',z[6],'EFFDEV',z[7],'OOS',z[4])
print('\nCLOSEST G75')
for i,(sc,c,p,z) in enumerate(sorted(res,key=lambda r:(abs(r[3][2][.75][0]-664),sum(abs(a-b) for a,b in zip(r[3][2][.75],TG[.75])),r[0]))[:30],1):print(i,'SCORE',sc,'C',c,'P',p,'DEV',z[1],'GATES',z[2],'EFFDEV',z[7],'OOS',z[4])
print('\nFULL ALPHA TOP25')
for rank,(sc,c,p,z) in enumerate(res[:25],1):
    aeall=0;tab={}
    for nominal,tgt in TA.items():
        ds=[];efs=[]
        for s in DEV:
            te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];pr,eff=fit(tr,te,nominal,c,p);y=te.y.to_numpy();ds.append(float(np.sqrt(np.mean(y*y))-np.sqrt(np.mean((y-pr)**2))));efs.append(eff)
        st=(min(ds),float(np.mean(ds)),float(np.median(ds)));er=sum(abs(a-b) for a,b in zip(st,tgt));aeall+=er;tab[nominal]=(st,er,efs)
    print('\nRANK',rank,'STAGE',sc,'C',c,'P',p,'ALPHAERR',aeall);print('ALPHAS',tab);print('GATES',z[2]);print('OOS',z[4])
