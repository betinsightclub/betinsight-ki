...
Z=Path('/tmp/refagg.sqlite.zip'); R=Path('/tmp/refaggdb'); DB=R/'basketball-final.sqlite'
if not DB.exists():
...
BASEPLUS=BASE.copy(); BASEPLUS['spread_move']=df.HomeSpread_AtClose-df.HomeSpread_AtOpen
...
def walk_one_season(X,s,alpha,mode):
    teall=df[df.season==s].sort_values('Date').copy()
    if mode=='season':
        tr=df[df.Date<pd.Timestamp(S[s][0])]
        c=fit(X,tr,teall,alpha)
        return pd.DataFrame({'idx':teall.index,'c':c,'y':teall.y.to_numpy(),'date':teall.Date.to_numpy()})
    keys=group_key(teall.Date,mode); outs=[]
    for k in keys.drop_duplicates():
        te=teall[keys==k]; cutoff=te.Date.min(); tr=df[df.Date<cutoff]
        c=fit(X,tr,te,alpha); outs.append(pd.DataFrame({'idx':te.index,'c':c,'y':te.y.to_numpy(),'date':te.Date.to_numpy()}))
    return pd.concat(outs,ignore_index=True)

def season_delta(z):
    y=z.y.to_numpy(); c=z.c.to_numpy(); return float(np.sqrt(np.mean(y*y))-np.sqrt(np.mean((y-c)**2)))
def bs(z,g):
    k=np.abs(z.c.to_numpy())>=g; yy=z.y.to_numpy()[k]; cc=z.c.to_numpy()[k]; r=np.sign(cc)*yy
    return (int(k.sum()),int((r>1e-12).sum()),int((r<-1e-12).sum()),int((np.abs(r)<=1e-12).sum()))

def evaluate(label,X,mode):
    print('\n===',label,mode,'===')
    ae=0; dev300=None
    for a,t in TA.items():
        zs=[walk_one_season(X,s,a,mode) for s in DEV]; ds=[season_delta(z) for z in zs]
        st=(min(ds),float(np.mean(ds)),float(np.median(ds))); er=sum(abs(u-v) for u,v in zip(st,t)); ae+=er
        print('ALPHA',a,'SEASON_DELTAS',ds,'SUMMARY',st,'TARGET',t,'ERR',er)
        if a==300:dev300=pd.concat(zs,ignore_index=True)
    print('ALPHAERR',ae); print('GATES')
    for g,t in TG.items():print(g,bs(dev300,g),'TARGET',t)
    print('OOS')
    for s,t in TO.items():
        z=walk_one_season(X,s,300,mode); rm=float(np.sqrt(np.mean((z.y.to_numpy()-z.c.to_numpy())**2))); print(s,'RMSE',repr(rm),'BETS',bs(z,.75),'TARGET',t)

for name,X in [('base8',BASE),('base8_spreadmove',BASEPLUS)]:
    for mode in ['season','month','week','quarter','30d']:
        evaluate(name,X,mode)
