import itertools, sqlite3, urllib.request, zipfile
from pathlib import Path
import numpy as np, pandas as pd
from sklearn.linear_model import Ridge
from sklearn.preprocessing import StandardScaler

URL='https://raw.githubusercontent.com/PranavSitaraman/NBA-Injury-Betting/main/basketball-final.sqlite.zip'
Z=Path('/tmp/form.sqlite.zip'); R=Path('/tmp/formdb'); DB=R/'basketball-final.sqlite'
if not DB.exists():
    R.mkdir(exist_ok=True); urllib.request.urlretrieve(URL,Z)
    with zipfile.ZipFile(Z) as z:z.extractall(R)
S={'2007-08':('2007-10-30','2008-04-16'),'2008-09':('2008-10-28','2009-04-15'),'2009-10':('2009-10-27','2010-04-14'),'2010-11':('2010-10-26','2011-04-13'),'2011-12':('2011-12-25','2012-04-26'),'2012-13':('2012-10-30','2013-04-17'),'2013-14':('2013-10-29','2014-04-16'),'2014-15':('2014-10-28','2015-04-15'),'2015-16':('2015-10-27','2016-04-13'),'2016-17':('2016-10-25','2017-04-12'),'2017-18':('2017-10-17','2018-04-11'),'2018-19':('2018-10-16','2019-04-10'),'2019-20':('2019-10-22','2020-08-14'),'2020-21':('2020-12-22','2021-05-16'),'2021-22':('2021-10-19','2021-12-20')}
DEV=['2013-14','2014-15','2015-16','2016-17']; OOS=['2017-18','2018-19','2019-20','2020-21','2021-22']
TA={10:(.009858199953276525,.03022947944542187,.03099032558293935),30:(.010448690894154566,.0300874616301412,.030785955809568222),100:(.01181166035632586,.029377116254404978,.02939163151319235),300:(.012949336036388814,.02695917735504505,.025827748983386023),1000:(.011899062924864978,.021358065322965913,.01913939031452916)}
TG={.25:(2841,1426,1345,70),.5:(1419,716,667,36),.75:(664,354,293,17),1:(302,158,136,8),1.25:(129,68,56,5),1.5:(50,29,19,2),2:(12,9,3,0)}
TO={'2017-18':(9.858216869241469,332,168,157,7),'2018-19':(10.149329971984457,295,155,131,9),'2019-20':(10.288966913270224,246,140,101,5),'2020-21':(10.481849926111051,195,99,91,5),'2021-22':(10.767341369515634,89,43,44,2)}
CT=np.array([-.8094746967645475,-.3325877729098592,-.27095763027679604,.24972916246690846,-.1656421906820333,.11024812552992498,.06462265416697556,-.04308359545971495])
NAMES=['margin2h_vs_pregame_half','ht_margin','ht_margin_surprise','spread_close','total2h_vs_pregame_half','total_close','total_move','ht_total_surprise']

con=sqlite3.connect(DB);o=pd.read_sql_query('select * from BettingOdds_History',con);d=pd.read_sql_query('select GAME_ID,PTS_QTR1_HOME,PTS_QTR2_HOME,PTS_QTR3_HOME,PTS_QTR4_HOME,PTS_QTR1_AWAY,PTS_QTR2_AWAY,PTS_QTR3_AWAY,PTS_QTR4_AWAY,PTS_OT1_HOME,PTS_OT2_HOME,PTS_OT3_HOME,PTS_OT4_HOME,PTS_OT5_HOME,PTS_OT1_AWAY,PTS_OT2_AWAY,PTS_OT3_AWAY,PTS_OT4_AWAY,PTS_OT5_AWAY from Game_FullDetails',con);con.close();d=d.drop_duplicates('GAME_ID')
r=o[o.GAME_ID.isin(set(d.GAME_ID))].copy()
for c in ['HomeSpread_AtOpen','HomeSpread_AtClose','Over_AtOpen','Over_AtClose','2H_HomeSpread','2H_Over']:r[c]=pd.to_numeric(r[c],errors='coerce')
r=r.dropna(subset=['HomeSpread_AtOpen','HomeSpread_AtClose','Over_AtOpen','Over_AtClose']);r=r[r.HomeSpread_AtOpen.abs().le(30)&r.HomeSpread_AtClose.abs().le(30)&r.Over_AtOpen.between(100,300)&r.Over_AtClose.between(100,300)];r=r.dropna(subset=['2H_HomeSpread','2H_Over']);r=r[r['2H_Over'].between(50,200)]
df=r.merge(d,on='GAME_ID',how='left',validate='many_to_one');q=['PTS_QTR1_HOME','PTS_QTR2_HOME','PTS_QTR3_HOME','PTS_QTR4_HOME','PTS_QTR1_AWAY','PTS_QTR2_AWAY','PTS_QTR3_AWAY','PTS_QTR4_AWAY']
for c in q:df[c]=pd.to_numeric(df[c],errors='coerce')
for side in ['HOME','AWAY']:
    for i in range(1,6):df[f'PTS_OT{i}_{side}']=pd.to_numeric(df[f'PTS_OT{i}_{side}'],errors='coerce').fillna(0)
df=df.dropna(subset=q).copy();df.Date=pd.to_datetime(df.Date);df['season']='other'
for s,(a,b) in S.items():df.loc[df.Date.between(pd.Timestamp(a),pd.Timestamp(b)),'season']=s
htm=(df.PTS_QTR1_HOME+df.PTS_QTR2_HOME)-(df.PTS_QTR1_AWAY+df.PTS_QTR2_AWAY); htt=df.PTS_QTR1_HOME+df.PTS_QTR2_HOME+df.PTS_QTR1_AWAY+df.PTS_QTR2_AWAY
actual=(df.PTS_QTR3_HOME+df.PTS_QTR4_HOME)-(df.PTS_QTR3_AWAY+df.PTS_QTR4_AWAY)+sum(df[f'PTS_OT{i}_HOME']-df[f'PTS_OT{i}_AWAY'] for i in range(1,6))
market=-df['2H_HomeSpread'];preg=-df.HomeSpread_AtClose;y=actual-market

M={
'half':market-preg/2,
'remaining':market-(preg-htm),
'full':market-preg,
}
H={
'half':htm-preg/2,
'full':htm-preg,
'open_half':htm-(-df.HomeSpread_AtOpen)/2,
}
T={
'half':df['2H_Over']-df.Over_AtClose/2,
'remaining':df['2H_Over']-(df.Over_AtClose-htt),
'full':df['2H_Over']-df.Over_AtClose,
'open_half':df['2H_Over']-df.Over_AtOpen/2,
}
U={
'half':htt-df.Over_AtClose/2,
'full':htt-df.Over_AtClose,
'open_half':htt-df.Over_AtOpen/2,
}
V={
'close_minus_open':df.Over_AtClose-df.Over_AtOpen,
'open_minus_close':df.Over_AtOpen-df.Over_AtClose,
'abs_move':(df.Over_AtClose-df.Over_AtOpen).abs(),
'h2_vs_open_half':df['2H_Over']-df.Over_AtOpen/2,
'final_total_reprice':df['2H_Over']+htt-df.Over_AtClose,
'h2_double_vs_close':2*df['2H_Over']-df.Over_AtClose,
}

def X_for(keys):
    mk,hk,tk,uk,vk=keys
    return pd.DataFrame({
    'margin2h_vs_pregame_half':M[mk], 'ht_margin':htm, 'ht_margin_surprise':H[hk], 'spread_close':df.HomeSpread_AtClose,
    'total2h_vs_pregame_half':T[tk], 'total_close':df.Over_AtClose, 'total_move':V[vk], 'ht_total_surprise':U[uk]},index=df.index)

def fit(X,tr,te,a):
    sc=StandardScaler();A=sc.fit_transform(X.loc[tr.index]);B=sc.transform(X.loc[te.index]);m=Ridge(alpha=a,fit_intercept=True).fit(A,y.loc[tr.index].to_numpy());return m.predict(B),m

def bstat(c, yy, g):
    k=np.abs(c)>=g;r=np.sign(c[k])*yy[k];return (int(k.sum()),int((r>1e-12).sum()),int((r<-1e-12).sum()),int((np.abs(r)<=1e-12).sum()))

def eval_fast(keys):
    X=X_for(keys);devpred=[];d300=[]
    for s in DEV:
        te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];c,_=fit(X,tr,te,300);devpred.append((te.index.to_numpy(),c));base=np.sqrt(np.mean(y.loc[te.index].to_numpy()**2));mod=np.sqrt(np.mean((y.loc[te.index].to_numpy()-c)**2));d300.append(base-mod)
    ids=np.concatenate([a for a,b in devpred]);c=np.concatenate([b for a,b in devpred]);yy=y.loc[ids].to_numpy(); ds=np.array([min(d300),np.mean(d300),np.median(d300)]); dse=float(np.abs(ds-np.array(TA[300])).sum())
    ge=0;g75=None
    for g,t in TG.items():
        b=bstat(c,yy,g);ge+=sum(abs(x-z) for x,z in zip(b,t));
        if g==.75:g75=b
    oe=0;orm=0;olist=[]
    for s,t in TO.items():
        te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];cc,_=fit(X,tr,te,300);rm=float(np.sqrt(np.mean((y.loc[te.index].to_numpy()-cc)**2)));bs=bstat(cc,y.loc[te.index].to_numpy(),.75);orm+=abs(rm-t[0]);oe+=sum(abs(x-z) for x,z in zip(bs,t[1:]));olist.append((s,rm,bs))
    score=dse*4000+ge*.12+orm*2200+oe*.10
    return score,ds,g75,ge,orm,oe,olist

res=[]
space=itertools.product(M,H,T,U,V)
for keys in space:
    z=eval_fast(keys);res.append((z[0],keys,z))
res.sort(key=lambda x:x[0])
print('CANDIDATES',len(res))
print('CURRENT_RANK',next(i+1 for i,x in enumerate(res) if x[1]==('half','half','half','half','close_minus_open')))
print('\nTOP30 FAST')
for i,(sc,k,z) in enumerate(res[:30],1): print(i,'SCORE',sc,'KEYS',k,'DEV',z[1],'G75',z[2],'GERR',z[3],'RMERR',z[4],'OOSBETERR',z[5],'OOS',z[6])

print('\nFULL TOP20')
for rank,(sc,k,z) in enumerate(res[:20],1):
    X=X_for(k);ae=0;ats={};dev300=[]
    for a,tgt in TA.items():
        ds=[]
        for s in DEV:
            te=df[df.season==s];tr=df[df.Date<pd.Timestamp(S[s][0])];c,_=fit(X,tr,te,a);base=np.sqrt(np.mean(y.loc[te.index].to_numpy()**2));mod=np.sqrt(np.mean((y.loc[te.index].to_numpy()-c)**2));ds.append(base-mod)
        st=np.array([min(ds),np.mean(ds),np.median(ds)]);e=float(np.abs(st-np.array(tgt)).sum());ae+=e;ats[a]=(st,e)
    tr=df[df.season.isin(DEV)];scal=StandardScaler();A=scal.fit_transform(X.loc[tr.index]);m=Ridge(alpha=300).fit(A,y.loc[tr.index].to_numpy());cd=float(np.linalg.norm(m.coef_-CT))
    final=sc+ae*3500+cd*12
    print('\nRANK',rank,'FINAL',final,'FAST',sc,'KEYS',k,'ALPHAERR',ae,'COEFDIST',cd);print('ALPHAS',ats);print('COEFS',dict(zip(NAMES,m.coef_)));print('DEV',z[1],'G75',z[2],'GERR',z[3]);print('OOS',z[6])
