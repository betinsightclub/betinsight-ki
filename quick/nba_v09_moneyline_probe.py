...
Z=Path('/tmp/mlp.sqlite.zip');R=Path('/tmp/mlpdb');DB=R/'basketball-final.sqlite'
if not DB.exists():
...
def ml_num(s):
    x=s.astype(str).str.strip().str.upper().replace({'EVEN':'100','EV':'100','PK':'100','PICK':'100','NONE':np.nan,'NAN':np.nan})
    return pd.to_numeric(x,errors='coerce')
def imp(a):
    a=np.asarray(a,dtype=float);return np.where(a<0,(-a)/((-a)+100),100/(a+100))
hml=ml_num(df.HomeML);aml=ml_num(df.AwayML);ph=pd.Series(imp(hml),index=df.index);pa=pd.Series(imp(aml),index=df.index);devig=ph/(ph+pa);logit=np.log(devig/(1-devig))
OPT=pd.DataFrame({'spread_move':df.HomeSpread_AtClose-df.HomeSpread_AtOpen,'spread_open':df.HomeSpread_AtOpen,'home_ml_raw':hml,'away_ml_raw':aml,'home_ml_imp':ph,'ml_devig_home':devig,'ml_devig_logit':logit},index=df.index)
print('ML_MISSING',OPT[['home_ml_raw','away_ml_raw']].isna().sum().to_dict(),'ML_EXAMPLES',list(zip(df.HomeML.head(10),df.AwayML.head(10),devig.head(10))))
...
def bs(c,y,g):
    k=np.abs(c)>=g;v=np.sign(c[k])*y[k];return (int(k.sum()),int((v>1e-12).sum()),int((v<-1e-12).sum()),int((np.abs(v)<=1e-12).sum()))

def evaluate(name,extras):
    X=pd.concat([BASE,OPT[extras]],axis=1) if extras else BASE.copy();good=X.notna().all(axis=1);dd=df[good].copy();X=X.loc[good]
    print('\n===',name,'EXTRAS',extras,'ROWS',len(dd),'===');ae=0;z300=None
    for a,t in TA.items():
        ds=[];zs=[]
        for s in DEV:
        ...
        st=(min(ds),float(np.mean(ds)),float(np.median(ds)));er=sum(abs(u-v) for u,v in zip(st,t));ae+=er
        if a==300:z300=pd.concat(zs,ignore_index=True)
        print('ALPHA',a,st,'ERR',er)
    print('ALPHAERR',ae,'GATES')
    for g,t in TG.items():print(g,bs(z300.c.to_numpy(),z300.y.to_numpy(),g),'TARGET',t)
    print('OOS')
    for s,t in TO.items():
        te=dd[dd.season==s];tr=dd[dd.Date<pd.Timestamp(S[s][0])];c,_=fit(X,tr,te,300);rm=float(np.sqrt(np.mean((te.y.to_numpy()-c)**2)));print(s,'RMSE',repr(rm),'BETS',bs(c,te.y.to_numpy(),.75),'TARGET',t)
    tr=dd[dd.season.isin(DEV)];sc=StandardScaler();A=sc.fit_transform(X.loc[tr.index]);m=Ridge(alpha=300).fit(A,tr.y.to_numpy());coef=dict(zip(X.columns,m.coef_));cv=np.array([coef[x] for x in DOM]);print('DOMDIST',float(np.linalg.norm(cv-CT)),'EXTRA_COEFS',{x:coef[x] for x in extras})

variants=[
('base8',[]),('spread_move',['spread_move']),('spread_open',['spread_open']),('devig',['ml_devig_home']),('logit',['ml_devig_logit']),('imp',['home_ml_imp']),('raw_home',['home_ml_raw']),('raw_both',['home_ml_raw','away_ml_raw']),
('spread_move_devig',['spread_move','ml_devig_home']),('spread_move_logit',['spread_move','ml_devig_logit']),('spread_open_devig',['spread_open','ml_devig_home']),('spread_open_logit',['spread_open','ml_devig_logit']),('spread_move_rawboth',['spread_move','home_ml_raw','away_ml_raw']),('spread_open_rawboth',['spread_open','home_ml_raw','away_ml_raw']),('spread_open_move_devig',['spread_open','spread_move','ml_devig_home'])]
for n,e in variants:evaluate(n,e)
